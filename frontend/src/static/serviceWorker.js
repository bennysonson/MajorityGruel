...
const STATIC_CACHE_NAME = 'mealdecider-static-v0';
  ...
  event.waitUntil(
    caches.open(STATIC_CACHE_NAME).then(cache => {
      return cache.addAll([
        ...
        //CSS
        '/css/addRecipe.css',
        '/css/browseRecipes.css',
        '/css/decisionRecipeSelect.css',
        '/css/home.css',
        '/css/quiz.css',
        '/css/styles.css',
        //Images
    ...
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.filter(cacheName => {
          return cacheName.startsWith('mealdecider-') && cacheName != STATIC_CACHE_NAME;
        }).map(cacheName => {
